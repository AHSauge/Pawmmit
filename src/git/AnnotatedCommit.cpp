...
AnnotatedCommit::operator git_annotated_commit *() const { return d.get(); }

Commit AnnotatedCommit::commit() const {
  git_commit *commit = nullptr;
  git_commit_lookup(&commit, repo, git_annotated_commit_id(d.get()));
  return Commit(commit);
}

int AnnotatedCommit::analysis() const {
  const git_annotated_commit *head = d.get();
